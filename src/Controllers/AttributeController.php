...
class AttributeController
{
    ...
    public function index(Request $request)
    {
        ...
        $categories = $this->attributeService->getGroupedAttributesByProduct($productId);
    ...
    public function store(Request $request)
    {
        ...
        $productId = $request->input('product_id');
        if ($productId) {
            $productExists = \EvolutionCMS\Models\SiteContent::where('id', $productId)->exists();
            if (!$productExists) {
    ...
    public function destroy($id)
    {
        ...
        $usedInValues = \roilafx\Product\Models\VariantAttributeValue::where('attribute_id', $id)->count();
        if ($usedInValues > 0) {
        ...
        $usedInPresets = \roilafx\Product\Models\AttributePresetAttribute::where('attribute_id', $id)->count();
        if ($usedInPresets > 0) {
